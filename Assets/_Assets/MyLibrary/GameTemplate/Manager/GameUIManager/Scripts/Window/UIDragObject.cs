...
public class UIDragObject : UIBehaviour, IEventSystemHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void OnBeginDrag(PointerEventData data)
    {
        ...
        RectTransformUtility.ScreenPointToLocalPointInRectangle(this.m_CanvasRectTransform, data.position,
            data.pressEventCamera, out this.m_PointerStartPosition);
        ...
        if (!this.m_Dragging && this.m_Velocity != Vector2.zero)
        {
            ...
            if (this.m_Velocity != Vector2.zero)
            {
                ...
                if (anchoredPosition != this.m_Target.anchoredPosition)
                {
                    ...
                    {
                        case Rounding.Hard:
                            this.m_Target.anchoredPosition = new Vector2(Mathf.Round(anchoredPosition.x / 2f) * 2f,
                                Mathf.Round(anchoredPosition.y / 2f) * 2f);
                            break;
                        case Rounding.Soft:
                        default:
                            this.m_Target.anchoredPosition = new Vector2(Mathf.Round(anchoredPosition.x),
